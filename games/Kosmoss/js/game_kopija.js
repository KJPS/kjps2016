window.onload = function(){
	...
	var now;
	var boostDual=false;var activDual=false;var dualTime;
	var bomb;
	var spark;
	var energy;
	var scor;
	...
	var enemymSpeed;
	var boost;var HP;var HPmade=false;
	var recoil;
	var HPtime;
	var shooting=false;
	var fon;
	...
	function enemy1(){
		var randomEnShipY = Math.random() * (window.innerHeight - 40);
		if(randomEnShipY<=31){
			randomEnShipY=32;
		}
	...
	function dualBoost(){
		var randomDualBoostY = Math.random() * (window.innerHeight - 100);
		if(randomDualBoostY<30){
			randomDualBoostY=30;
		}
		boost=image(window.innerWidth, randomDualBoostY, 'dual.png');
		
	}
	function shieldBoost(){
		var randomShieldBoostY = Math.random() * (window.innerHeight - 100);
		if(randomShieldBoostY<30){
			randomShieldBoostY=30;
		}
		HP=image(window.innerWidth-40, randomShieldBoostY, 'shield.png');
	...
	animate(function(){
		if(game==true){
			...
			if(bomb.position.x!=-100){
				moveBy(bomb,-1.5*galaxySpeed,0);
			}
			//spark movements
			if(spark.position.x!=-100){
				moveBy(spark,-1.5*galaxySpeed,0);
			}
			//laserShot from ship
			...
			for(var i=0;i<laserEnShot.length;i++) {
				...
				if(isCollision(laserEnShot[i],ship,0)){
					...
					}if(boostDual==true && shield.length==1){
			...
			for(var i=0;i<enemy.length;i++){
				...
				moveBy(enemy[i],-1.5*galaxySpeed,enemyMoveY[i]);
			...
			//spawns a dualBoost
			now = new Date().getTime();
			if(now-dualTime>=9000 && activDual==false){
				dualBoost();
				activDual=true;
				dualTime = new Date().getTime();
			}
			if(activDual==true){
				...
				if(isCollision(ship, boost, 0)){
					...
					moveBy(boost,-2.5*galaxySpeed,0);
				}
			}
			...
			now = new Date().getTime();
			if(now-HPtime>=12000 && HPmade==false){
				HPmade=true;
				shieldBoost();
				HPtime = new Date().getTime();
			}
			if(HPmade==true && shieldPOW.position.x+100+16*shield.length+15<window.innerWidth){
				//shieldBoost hits my ship
				if(isCollision(ship, HP, 0)){
					HPmade=false;
					remove(HP);
					...
					HPUP();
				}else if(HP.position.x==-75){
					HPmade=false;
					remove(HP);
				}else{
					moveBy(HP,-2.5*galaxySpeed,0);
				}
			}
		}
	});
